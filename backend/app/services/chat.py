...
from app.schemas.message import FileResponse, MessageResponse
...
class ChatService:
    """
    Service for chat operations: session validation, file handling,
    message creation, tool orchestration, and LLM responses.
    """
    ...
    async def validate_and_process_file(
        ...
        try:
            ...
            if file_size > self.max_file_size:
                ...
                return None, None, None, f"File {file_name} exceeds maximum size of {self.max_file_size / (1024 * 1024):.0f}MB"
    ...
    async def process_uploaded_files(
        ...
        if len(files) > self.max_files:
            logger.warning(
                f"Maximum {self.max_files} files allowed - session_id: {session_id}, "
                f"user_id: {user_id}, provided: {len(files)}"
            )
            return None, None, None, f"Maximum {self.max_files} files are allowed"
        ...
        for file in files:
            file_item, file_content, file_path, error_message = await self.validate_and_process_file(
