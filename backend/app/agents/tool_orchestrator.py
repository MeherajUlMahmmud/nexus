...
from app.prompts import TOOL_SELECTION_SYSTEM_PROMPT, get_tool_selection_prompt
...
class ToolOrchestrator:
    ...
    async def decide_tools(
        ...
        if not available_tools:
            logger.info("[TOOL_DECISION] No tools available in registry, skipping tool decision")
            return []
        ...
        try:
            ...
            logger.debug(f"[TOOL_DECISION] LLM response received - length: {len(content) if content else 0}")
            if not content:
                logger.warning("[TOOL_DECISION] Agent returned empty response, no tools will be called")
                return []
            ...
            try:
                ...
                if isinstance(decision_data, dict) and "tools" in decision_data:
                    ...
                    logger.warning(f"Unexpected decision format: {decision_data}")
                    return []
                ...
                for tool_data in tools_list:
                    if isinstance(tool_data, dict):
                        ...
                        sequence = tool_data.get("sequence", len(tool_decisions))
                        ...
                        if tool_name and tool_name in available_tools:
                            ...
                            logger.warning(f"Invalid tool name in decision: {tool_name}")
                ...
                logger.error(f"[TOOL_DECISION] Failed to parse agent response as JSON: {e}\nResponse: {content[:500]}")
                return []
                
        except Exception as e:
            logger.error(f"[TOOL_DECISION] Error in agent tool decision: {e}", exc_info=True)
            return []
    ...
    async def execute_tools(
        ...
        Args:
            ...
            request_context: Optional context (e.g., IP address)
        ...
        logger.info(f"[TOOL_EXECUTION] Starting execution of {len(tool_decisions)} tool(s) in sequence")
        
        for idx, decision in enumerate(tool_decisions, 1):
            tool = tool_registry.get(decision.tool_name)
            if not tool:
                logger.error(f"[TOOL_EXECUTION] Tool '{decision.tool_name}' not found in registry - sequence: {idx}/{len(tool_decisions)}")
                results.append({
            ...
            try:
                ...
                if request_context:
                    ...
                    for key, value in request_context.items():
                        if key in tool_params:
                            ...
                            logger.debug(f"[TOOL_EXECUTION] Skipping context parameter '{key}' - not in tool schema for {decision.tool_name}")
                ...
                logger.error(f"[TOOL_EXECUTION] Tool {decision.tool_name} failed after {tool_time:.2f}s: {error_msg}", exc_info=True)
                results.append({
                    "tool_name": decision.tool_name,
                    "success": False,
...
Be conversational and don't just list the tool results - synthesize them into a natural answer."""
        ...
        try:
            ...
            if not content:
                logger.warning("[RESPONSE_FORMATTING] LLM returned empty response, using fallback formatter")
                content = self._format_fallback_response(user_query, tool_results)
            ...
            logger.error(f"[RESPONSE_FORMATTING] Error formatting response: {e}", exc_info=True)
